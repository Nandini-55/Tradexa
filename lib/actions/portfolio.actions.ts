...
export async function executeTrade(
    ...
    const totalCostNative = quantity * currentPrice;
    const totalCostUSD = isIndian ? (totalCostNative * INR_TO_USD) : totalCostNative;

    if (type === 'BUY') {
        if (portfolio.balance < totalCostUSD) {
            return {
                success: false,
                error: `Insufficient coins. Needed 🪙 ${totalCostUSD.toFixed(2)}, Available: 🪙 ${portfolio.balance.toFixed(2)}`
            };
        }

        // Update USD balance
        portfolio.balance -= totalCostUSD;

        // Update holdings in native stock currency
        ...
        if (existingHoldingIndex > -1) {
            ...
            // Calculate new weighted average price in native currency
            const totalValueNative = (holding.quantity * holding.averagePrice) + totalCostNative;
            const newQuantity = holding.quantity + quantity;

            holding.averagePrice = totalValueNative / newQuantity;
    ...
    // Log Transaction in native currency
